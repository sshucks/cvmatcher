...
tmux send-keys -t devsession 'python -m fastapi dev application/api_call.py --host 0.0.0.0 --port 8000' C-m

# Split the window and run Streamlit in the second pane
tmux split-window -h -t devsession
tmux send-keys -t devsession 'python -m streamlit run application/matching_app.py --server.address=0.0.0.0' C-m
