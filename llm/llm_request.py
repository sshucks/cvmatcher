...
from definitions import CVData, RequirementsData, EducationData, ProfessionalExperienceData
...
class MatchingRequest(Request):
    ...
    def make_request(self, cv:CVData|list[str]|ProfessionalExperienceData|EducationData, requirements:RequirementsData|list[str]|ProfessionalExperienceData|EducationData):
        """
        Make the matching request to the LLM
        :param cv: (part of the) CV to match
        :param requirements: (part of the) requirements to match against
        :return: JSON object if successful or None in case of errors
        """
        url = self.model_manager.get_uri()

        payload = {"cv": cv, "requirements": requirements}
