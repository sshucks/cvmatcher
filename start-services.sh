...
tmux send-keys -t devsession 'PYTHONPATH=./src python -m fastapi dev src/api/api_call.py' C-m

# Split the window and run Streamlit in the second pane
tmux split-window -h -t devsession
tmux send-keys -t devsession 'PYTHONPATH=./src python -m streamlit run src/streamlit/matching_app.py' C-m
