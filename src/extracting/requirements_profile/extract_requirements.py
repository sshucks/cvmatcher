...
relevant_headings = [["Erfolgskritische Faktoren für die Position"],
                     ["Aufgaben"],
                     ["Fachliche Qualifikationen und Kompetenzen", "Fachliche Anforderungen"],
...
def is_qualification_heading(text: str):
    return text in qualification_parts
...
def extract_qualifications(skill_str: str):
    ...
    for i in range(len(splitted)):
        if splitted[i].strip() in relevant_parts:
            j = i + 1
...
def extract_eductaion(skill_str: str):
    relevant_parts = ["Formale Ausbildung", "Formale Ausbildung:"]
    splitted = skill_str.split("\n")
    skill_results = []
    for i in range(len(splitted)):
        if splitted[i].strip() in relevant_parts:
